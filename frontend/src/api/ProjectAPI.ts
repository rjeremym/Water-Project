...
export const fetchProjects = async (pageSize: number, pageNum: number, selectedCategories: string[]): Promise<FetchProjectsResponse> => {
    try {
        ...
        const response = await fetch(`${API_BASE_URL}/AllProjects?pageSize=${pageSize}&pageNum=${pageNum}${selectedCategories.length ? `&${categoryParams}` : ''}`);
        if (!response.ok) {
...
export const addProject = async (newProject: Project): Promise<Project> => {
    try {
        ...
        if (!response.ok) {
            throw new Error(`Failed to add project. HTTP status: ${response.status}`);
        }
        return await response.json();
    } catch (error) {
...
export const updateProject = async (projectId: number, updatedProject: Project): Promise<Project> => {
    try {
        ...
        if (!response.ok) {
            throw new Error(`Failed to update project. HTTP status: ${response.status}`);
        }
        return await response.json();
    } catch (error) {
